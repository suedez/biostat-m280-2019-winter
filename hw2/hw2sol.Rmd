...
# this funcion is used to check the validity of inputs, give warning message,
# and remove all invalid inputs
  x <- as.vector(x)
...
fizzbuzz <- function(x){
  ...
  kable(results_table, align = 'c')
    ...
    e) Print out redults with a nice format
...
2. Use your FizzBuzz function to display the correct output for a variety of inputs, make sure to show what happens for both good and bad input. 
    ...
    **SOLUTION**
    ...
    The fizzbuzz function will transform input into a vector and remove all invalid inputs with valid inputs left, let's try on semi-invalid and good inputs.
...
2. Re-do FizzBuzz test using Rcpp. 
    ...
    **SOLUTION**
    ...
    a) Let's look at the Rcpp file
    ```{bash}
    cat fizzbuzz.cpp
    ```
    
    b) Let's run it with semi-valid and good inputs
    ```{r}
    library(Rcpp)
    sourceCpp('fizzbuzz.cpp')
    ## semi-valid input
    inputx <- c(1.1, 2.8, 'ucla', 5, 6, 2)
    rcpp_fizzbuzz(inputx)
    ```
