...
```{r}
##try
filter(city_ep, Year == 2013) %>% select(Base_Pay, Overtime_Pay, Other_Pay) %>% 
apply(2, sum) -> try1
selected <- data.frame(type = c("base", "over", "other"), payment = try1)
barplot(selected$payment, names.arg = selected$type)
ggplot(data = selected, aes(x = type, y = payment), bar)
head(payroll_selected)
```
