...
3. Using your favorite text editor (e.g., `vi`), type the following and save the file as `middle.sh`:
    ...
    **SOLUTION**
    ...
    a. After edit, let't look at the file `middle.sh`
    ```{bash}
    cat middle.sh
    ```
    ...
    c. Explain the output. Explain the meaning of `"$1"`, `"$2"`, and `"$3"` in this shell script. Why do we need the first line of the shell script?
...
for both methods.
    ...
    **SOLUTION**
    ...
    a. Expand the `runSim.R` as it required, and let's have a look at the expanded `runSim.R`
    ```{bash}
    cat runSim.R
    ```
    
     b. Run the R code with certain arguments passed from linux
    ```{bash}
...
2. Modify the [`autoSim.R`](http://hua-zhou.github.io/teaching/biostatm280-2019winter/slides/02-linux/autoSim.R) script to run simulations with combinations of sample sizes `nVals = seq(100, 500, by=100)` and distributions `distTypes = c("gaussian", "t1", "t5")` and write output to appropriately named files. Use `rep = 50`, and `seed = 280`. 
    ...
    **SOLUTION**
    ...
    a. Modify the `autoSim.R` to meet the requirements and have a look on that R scripts
    ```{bash}
    cat autoSim.R
    ```
