...
for both methods.
  **SOLUTION**
  1)Download the original R script and write it into a `.R` file
```{bash,eval=FALSE}
curl http://hua-zhou.github.io/teaching/biostatm280-2019winter/slides/02-linux/runSim.R > runSim.R
```
  2) Expand the `runSim.R` as it required, and let's have a look at the expanded `runSim.R`
```{bash}
cat runSim.R
```
  3) Run the R code with certain arguments passed from linux
```{bash}
Rscript runSim.R  n=100 dist=\"t1\" rep=50 seed=280 # pass string to R
```
...
2. Modify the [`autoSim.R`](http://hua-zhou.github.io/teaching/biostatm280-2019winter/slides/02-linux/autoSim.R) script to run simulations with combinations of sample sizes `nVals = seq(100, 500, by=100)` and distributions `distTypes = c("gaussian", "t1", "t5")` and write output to appropriately named files. Use `rep = 50`, and `seed = 280`. 
  **SOLUTION**
  1)Download the original R script and write it into a `.R` file
```{bash eval=FALSE}
curl http://hua-zhou.github.io/teaching/biostatm280-2019winter/slides/02-linux/autoSim.R > autoSim.R
```
  2）Modify the `autoSim.R` to meet the requirements and have a look on that R scripts
```{bash}
cat autoSim.R
```
  3) Run the R code 
